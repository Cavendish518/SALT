...
Viewport::Viewport(QWidget* parent, Qt::WindowFlags f)
    : QGLWidget(parent, 0, f),
      ...
      mRadius(5),
      buttonPressed(false),
      texLabelColors_(32768,1, TextureFormat::RGB),
...
void Viewport::updateLabels() {
  ...
  while (count * max_size < bufLabels_.size()) {
    ...
    for (uint32_t i = 0; i < size; ++i) {
      ...
      labeledCount_ += (labels[i] != 0 && ((labels[i]&0xffff)<=1024));
    }
...
void Viewport::setLabelColors(const std::map<uint32_t, glow::GlColor>& colors) {
  ...
  std::vector<uint8_t> label_colors(3 * 32768, 0);
...
void Viewport::setFilteredLabels(const std::vector<uint32_t>& labels) {
  ...
  for (auto it = difference.begin(); it != end; ++it){
      setLabelVisibility(*it, 1);
//      if(*it == 0){
//      for (uint32_t fliter_id = 1025; fliter_id < 32768; fliter_id++){
//          setLabelVisibility(fliter_id,1);
//      }
//      }
  } // now visible again.
  ...
  for (auto it = difference.begin(); it != end; ++it){
      setLabelVisibility(*it, 0);
//      if(*it == 0){
//      for (uint32_t fliter_id = 1025; fliter_id < 32768; fliter_id++){
//          setLabelVisibility(fliter_id,0);
//      }
//      }
  }  // now invisible.
...
void Viewport::mousePressEvent(QMouseEvent* event) {
  ...
  if (labelInstances_) {
    ...
    if (mMode == PAINT) {
      ...
      updateGL();
    }else if (mMode == MERGE) {  // 处理 MERGE 模式
        if (event->buttons() & Qt::LeftButton) {
            int elapsed = lastClickTime.elapsed();  // 计算距离上次点击的时间（毫秒）
            lastClickTime.start();  // 更新点击时间

            if (elapsed < 500) {  // 500 毫秒内的两次点击才算有效
                clickCount++;
            } else {
                clickCount = 1;  // 超过时间限制，重新开始计数
            }

            if (clickCount == 2) {  // 达到两次点击，触发合并
                uint32_t mergelabel = getClickedLabels(event->x(), event->y(),mCurrentLabel);
                std::cout<<"merge "<<mergelabel<<std::endl;
                if((mergelabel&uint32_t(0xffff)) > 1024) labelPoints(event->x(), event->y(), mergelabel, mCurrentLabel, false);
                updateGL();
//                std::cout<<"k"<<std::endl;
                clickCount = 0;  // 重置计数
            }
...
uint32_t Viewport::getClickedLabels(int32_t x, int32_t y,uint32_t new_label) {
  std::vector<uint32_t> selected;

  if (points_.size() == 0 || labels_.size() == 0) return 0;
  ...
  prgGetSelectedLabels_.setUniform(mvp_);
  //prgGetSelectedLabels_.setUniform(GlUniform<int32_t>("numTriangles", numTriangles_));
  prgGetSelectedLabels_.setUniform(GlUniform<int32_t>("labelingMode", 2));
  prgGetSelectedLabels_.setUniform(GlUniform<vec2>("window_pos", glow::vec2(x, y)));
  ...
  float max_z = float(0xFFFF);
  for (uint32_t label : selected) {
      // 提取高16位对应的 pos.z
      if (new_label == (label & 0xFFFF)) continue;
      uint32_t pos_z = (label >> 16) & 0xFFFF; // 高16位对应的 pos.z
      float z = float(pos_z); // 反向转换为 pos.z
      std::cout<<"z "<<z<<"label "<<(label & 0xFFFF) <<std::endl;
      if (max_z > z){
          max_z = z;
          out_label = label & 0xFFFF;
      }
  }
  if (out_label == 0) out_label = new_label;
...
void Viewport::labelPoints(int32_t x, int32_t y, float radius, uint32_t new_label, bool remove) {
  ...
  if (mMode == Viewport::MERGE) prgUpdateLabels_.setUniform(GlUniform<int32_t>("labelingMode", 2));
...
void Viewport::updateBoundingBoxes() {
  ...
  bool showSingleScan = drawingOption_["single scan"];
  bool showScanRange = drawingOption_["show scan range"];
  uint32_t buffer_label_max_size = bufLabels_.size();
  if (showSingleScan){
    count = scanInfos_[singleScanIdx_].start;
    buffer_label_max_size = scanInfos_[singleScanIdx_].start + scanInfos_[singleScanIdx_].size;
  }
  else if (showScanRange){
    count = scanInfos_[scanRangeBegin_].start;
    buffer_label_max_size =
      scanInfos_[scanRangeEnd_].start + scanInfos_[scanRangeEnd_].size;
  }
  ...
  float progress = 0.1;
  while (count  < buffer_label_max_size) {
    if (count  > progress * buffer_label_max_size) {
      std::cout << "=" << std::flush;
      progress += 0.1;
    }

    uint32_t size = std::min<uint32_t>(max_size, buffer_size - count);

    bufLabels_.copyTo(count, size, bufReadLabels, 0);
    bufPoints_.copyTo(count, size, bufReadPoints, 0);
    bufScanIndexes_.copyTo(count, size, bufReadIndexes, 0);
